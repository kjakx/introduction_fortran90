...
contains
    ...
    function chk_err(phi, phi2, n1, n2, dt) result(er)
        double precision, intent(in) :: phi(:, :), phi2(:, :), dt
        integer, intent(in) :: n1, n2
    ...
    subroutine time_dependent_dispersion_explicit(phi, x, n1, n2, dt, alpha, n_step, p_step, er0)
        ...
        do i_step = 1, n_step
            ...
            call set_nbc(phi_next, n1, n2)
            er = chk_err(phi, phi_next, n1, n2, dt)
            phi(:, :) = phi_next(:, :)
            if (er < er0) exit
        end do
        ...
        double precision dx1, dx2, d1, d2, gamma, e, f, rhs, er, phi2(n1, n2)
        integer i, j, i_step
        integer :: p_count = 0
        character(10000) :: filename
        dx1 = 1.0d0 / dble(n1 - 1)
        ...
        do i_step = 1, n_step
            do j = 2, n2 - 1
                do i = 2, n1 - 1
                    ...
                    phi2(i, j) = phi(i, j) + ohm * (rhs - (1.0d0 - gamma) * phi(i, j))
                end do
            end do
            call set_nbc(phi2, n1, n2)
            er = chk_err(phi, phi2, n1, n2, dt)
            phi(:, :) = phi2(:, :)
            if (er < er0) exit
        end do
...
program main
    ...
    call time_dependent_dispersion_explicit(phi, x, n1, n2, dble(1.0e-2), alpha, n_step, p_step, er0)
    call cpu_time(t_end)
